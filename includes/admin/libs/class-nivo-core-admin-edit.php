...
class Nivo_Core_Admin_Edit extends Nivo_Core_Abstract implements Nivo_Library_Interface {
	...
	public function admin_init() {
		...
		add_meta_box(
			$this->labels['post_type'] . '_usefullinks_box', __( 'Useful Links', 'nivo-slider' ), array(
			    $this,
	...
	public function meta_box_usefullinks() {
		...
		echo '<p>' . __( 'Website:', 'nivo-slider' ) . ' <a href="' . TI_SITE_URL . '/products/' . $this->labels['slug'] . '/" target="_blank">' . $this->labels['plugin_name'] . '</a></p>';
		echo '<p>' . __( 'Created by:', 'nivo-slider' ) . ' <a href="' . TI_SITE_URL . '" target="_blank">ThemeIsle</a></p>';
		if ( ! $this->is_lite ) {
			echo '<p>' . __( 'Support:', 'nivo-slider' ) . ' <a href="' . TI_SITE_URL . '/support" target="_blank">Support</a></p>';
		}
		echo '<p>' . __( 'Documentation:', 'nivo-slider' ) . ' <a href="' . $this->get_documentation_link() . '" target="_blank">Documentation</a></p>';
		echo '<p>' . __( 'Changelog:', 'nivo-slider' ) . ' <a href="' . $this->labels['plugin_url'] . 'changelog.txt" target="_blank">Changelog</a></p>';
		echo '</div>';
	}
	...
	public function meta_box_upload() {
		...
		<input type="hidden" id="_manual_image_ids" name="<?php echo $this->labels['post_meta_key']; ?>[manual_image_ids]"
				...
				<td>
					...
					<?php _e( 'Choose to manually upload images or use images from the media library.', 'nivo-slider' ); ?>
						...
						if ( ! $this->nivo_mmp_active() ) {
							printf( __( 'You can add images from external sources and use them as feeds with our free plugin %1$s Media Manager Plus %2$s', 'nivo-slider' ), '<a href="' . admin_url( 'edit.php?post_type=' . $this->labels['post_type'] . '&page=' . $this->labels['post_type'] . '-install-plugins' ) . '">', '</a>' );
						} else {
							printf( __( 'Connect more sources using %1$s Media Manager Plus %2$s', 'nivo-slider' ), '<a href="' . admin_url( 'upload.php?page=uber-media' ) . '">', '</a>' );
						}
						?>
						<br>
