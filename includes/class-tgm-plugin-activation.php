...
if ( ! class_exists( 'TGM_Nivoslider_Plugin_Activation' ) ) {
	/**
 	 ...
 	 * @author Gary Jones <gamajo@gamajo.com>
 	 */
	class TGM_Nivoslider_Plugin_Activation {
		...
		/**
	 	 ...
	 	 * @since 1.0.0
	 	 *
	 	 * @var TGM_Nivoslider_Plugin_Activation
	 	 */
		static $instance;
		 ...
		 * does the TGMPA_Nivoslider_init action hook, and hooks in the interactions to init.
		 *
		 * @since 1.0.0
		 *
		 * @see TGM_Nivoslider_Plugin_Activation::init()
		 */
		public function __construct() {
			...
			$this->strings = array(
				...
				'no_items_after'				  => __( 'Return to the Dashboard', $this->domain ),
			);

			/** Annouce that the class is ready, and pass the object (for advanced use) */
			do_action_ref_array( 'TGMPA_Nivoslider_init', array( &$this ) );
		 ...
		 * @since 2.0.0
		 *
		 * @see TGM_Nivoslider_Plugin_Activation::admin_menu()
		 * @see TGM_Nivoslider_Plugin_Activation::notices()
		 * @see TGM_Nivoslider_Plugin_Activation::styles()
		 */
		public function init() {

			do_action( 'tgmpa_nivoslider_register' );
			...
			if ( $this->plugins ) {
				...
				if ( $this->is_TGMPA_Nivoslider_page() ) {
		...
		public function thickbox() {
			...
			if ( ! get_user_meta( get_current_user_id(), 'TGMPA_Nivoslider_dismissed_notice', true ) )
				add_thickbox();
		 ...
		 *
		 * @see TGM_Nivoslider_Plugin_Activation::init()
		 * @see TGM_Nivoslider_Plugin_Activation::install_plugins_page()
		 */
		...
		public function install_plugins_page() {
			...
			$plugin_table = new TGMPA_Nivoslider_List_Table;
			...
			<div class="tgmpa wrap">
				...
				<?php screen_icon( apply_filters( 'TGMPA_Nivoslider_default_screen_icon', 'themes' ) ); ?>
			...
			if ( ! get_user_meta( get_current_user_id(), 'TGMPA_Nivoslider_dismissed_notice', true ) ) {
				...
				if ( ! empty( $message ) ) {
					...
					$action_links = apply_filters(
						'TGMPA_Nivoslider_notice_action_links',
						array(
		...
		public function dismiss() {
			...
			if ( isset( $_GET[sanitize_key( 'tgmpa-dismiss' )] ) )
				update_user_meta( get_current_user_id(), 'TGMPA_Nivoslider_dismissed_notice', 1 );
		 ...
		 */
		protected function is_TGMPA_Nivoslider_page() {
		...
		public function update_dismiss() {
			...
			delete_user_meta( get_current_user_id(), 'TGMPA_Nivoslider_dismissed_notice' );
...
new TGM_Nivoslider_Plugin_Activation;

if ( ! function_exists( 'tgmpa_nivoslider' ) ) {
	/**
	 ...
	 */
	function tgmpa_nivoslider( $plugins, $config = array() ) {

		foreach ( $plugins as $plugin )
			TGM_Nivoslider_Plugin_Activation::$instance->register( $plugin );

		if ( $config )
			TGM_Nivoslider_Plugin_Activation::$instance->config( $config );
...
if ( ! class_exists( 'TGMPA_Nivoslider_List_Table' ) ) {
	/**
	 ...
	 * @author Gary Jones <gamajo@gamajo.com>
	 */
	class TGMPA_Nivoslider_List_Table extends WP_List_Table {

		/**
		 * References parent constructor and sets defaults for class.
		 *
		 * The constructor also grabs a copy of $instance from the TGMPA class
		 * and stores it in the global object TGM_Nivoslider_Plugin_Activation::$instance.
		 *
		 * @since 2.2.0
		 *
		...
		protected function _gather_plugin_data() {
			...
			TGM_Nivoslider_Plugin_Activation::$instance->admin_init();
			TGM_Nivoslider_Plugin_Activation::$instance->thickbox();
			...
			foreach ( TGM_Nivoslider_Plugin_Activation::$instance->plugins as $plugin ) {
				if ( is_plugin_active( $plugin['file_path'] ) )
				...
				if ( isset( $plugin['external_url'] ) ) {
					...
					$table_data[$i]['source'] = __( 'External Link', TGM_Nivoslider_Plugin_Activation::$instance->domain );
				}
				elseif ( isset( $plugin['source'] ) ) {
					/** The plugin must be from a private repository */
					if ( preg_match( '|^http(s)?://|', $plugin['source'] ) )
						$table_data[$i]['source'] = __( 'Private Repository', TGM_Nivoslider_Plugin_Activation::$instance->domain );
					/** The plugin is pre-packaged with the theme */
					else
						$table_data[$i]['source'] = __( 'Pre-Packaged', TGM_Nivoslider_Plugin_Activation::$instance->domain );
				}
				/** The plugin is from the WordPress repository */
				else {
					$table_data[$i]['source'] = __( 'WordPress Repository', TGM_Nivoslider_Plugin_Activation::$instance->domain );
				}

				$table_data[$i]['type'] = $plugin['required'] ? __( 'Required', TGM_Nivoslider_Plugin_Activation::$instance->domain ) : __( 'Recommended', TGM_Nivoslider_Plugin_Activation::$instance->domain );

				if ( ! isset( $installed_plugins[$plugin['file_path']] ) )
					$table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', TGM_Nivoslider_Plugin_Activation::$instance->domain ) );
				elseif ( is_plugin_inactive( $plugin['file_path'] ) )
					$table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', TGM_Nivoslider_Plugin_Activation::$instance->domain ) );
		 ...
		 * TGM_Nivoslider_Plugin_Activation class.
		 *
		 * Loops through the registered plugins looking for $name. If it finds it,
		 * it returns the $data from that plugin. Otherwise, returns false.
		 *
		...
		protected function _get_plugin_data_from_name( $name, $data = 'slug' ) {
			...
			foreach ( TGM_Nivoslider_Plugin_Activation::$instance->plugins as $plugin => $values ) {
				if ( $name == $values['name'] && isset( $values[$data] ) )
					return $values[$data];
			}
				...
				$actions = array(
					'install' => sprintf(
						...
						wp_nonce_url(
							esc_url_raw( add_query_arg(
								array(
									'page'          => TGM_Nivoslider_Plugin_Activation::$instance->menu,
									'plugin'        => $item['slug'],
									'plugin_name'   => $item['sanitized_plugin'],
									'plugin_source' => $item['url'],
									'tgmpa-install' => 'install-plugin',
								),
								admin_url( TGM_Nivoslider_Plugin_Activation::$instance->parent_url_slug )
							) ),
							'tgmpa-install'
						),
		...
		public function no_items() {
			...
			printf( __( 'No plugins to install or activate. <a href="%1$s" title="%2$s">%2$s</a>', TGM_Nivoslider_Plugin_Activation::$instance->domain ), admin_url( TGM_Nivoslider_Plugin_Activation::$instance->parent_url_slug ),  TGM_Nivoslider_Plugin_Activation::$instance->strings['no_items_after']);
			echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
		...
		public function get_columns() {
			...
			$columns = array(
				...
				'plugin' => __( 'Plugin', TGM_Nivoslider_Plugin_Activation::$instance->domain ),
				'source' => __( 'Source', TGM_Nivoslider_Plugin_Activation::$instance->domain ),
				'type'   => __( 'Type', TGM_Nivoslider_Plugin_Activation::$instance->domain ),
				'status' => __( 'Status', TGM_Nivoslider_Plugin_Activation::$instance->domain )
			);
		...
		public function get_bulk_actions() {
			...
			$actions = array(
				'tgmpa-bulk-install'  => __( 'Install', TGM_Nivoslider_Plugin_Activation::$instance->domain ),
				'tgmpa-bulk-activate' => __( 'Activate', TGM_Nivoslider_Plugin_Activation::$instance->domain ),
			);
		...
		public function process_bulk_actions() {
			...
			if ( 'tgmpa-bulk-install' === $this->current_action() ) {
				...
				if ( is_wp_error( $api ) )
					wp_die( TGM_Nivoslider_Plugin_Activation::$instance->strings['oops'] . var_dump( $api ) );
				...
				$url   = esc_url_raw( add_query_arg( array( 'page' => TGM_Nivoslider_Plugin_Activation::$instance->menu ), admin_url( TGM_Nivoslider_Plugin_Activation::$instance->parent_url_slug ) ) );
				$nonce = 'bulk-plugins';
				$names = $plugin_names;

				/** Create a new instance of TGM_Nivoslider_Bulk_Installer */
				$installer = new TGM_Nivoslider_Bulk_Installer( $skin = new TGM_Nivoslider_Bulk_Installer_Skin( compact( 'url', 'nonce', 'names' ) ) );

				/** Wrap the install process with the appropriate HTML */
				echo '<div class="tgmpa wrap">';
					screen_icon( apply_filters( 'TGMPA_Nivoslider_default_screen_icon', 'themes' ) );
			...
			if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
				...
				if ( is_wp_error( $activate ) )
					...
					printf( '<div id="message" class="updated"><p>%1$s %2$s</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, TGM_Nivoslider_Plugin_Activation::$instance->domain ), $imploded );
 ...
 * @since 2.2.0
 */
if ( ! class_exists( 'WP_Upgrader' ) && ( isset( $_GET[sanitize_key( 'page' )] ) && TGM_Nivoslider_Plugin_Activation::$instance->menu = $_GET[sanitize_key( 'page' )] ) ) {
	require_once ABSPATH . 'wp-admin/includes/class-wp-upgrader.php';

	if ( ! class_exists( 'TGM_Nivoslider_Bulk_Installer' ) ) {
		/**
		 ...
		 * @author Gary Jones <gamajo@gamajo.com>
		 */
		class TGM_Nivoslider_Bulk_Installer extends WP_Upgrader {
			...
			public function bulk_install( $packages ) {
				...
				if ( TGM_Nivoslider_Plugin_Activation::$instance->is_automatic )
			...
			public function run( $options ) {
				...
				if ( TGM_Nivoslider_Plugin_Activation::$instance->is_automatic ) {
					...
					TGM_Nivoslider_Plugin_Activation::$instance->populate_file_path();
			...
			public function install_strings() {
				...
				$this->strings['no_package']          = __( 'Install package not available.', TGM_Nivoslider_Plugin_Activation::$instance->domain );
				$this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', TGM_Nivoslider_Plugin_Activation::$instance->domain );
				$this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', TGM_Nivoslider_Plugin_Activation::$instance->domain );
				$this->strings['installing_package']  = __( 'Installing the plugin&#8230;', TGM_Nivoslider_Plugin_Activation::$instance->domain );
				$this->strings['process_failed']      = __( 'Plugin install failed.', TGM_Nivoslider_Plugin_Activation::$instance->domain );
				$this->strings['process_success']     = __( 'Plugin installed successfully.', TGM_Nivoslider_Plugin_Activation::$instance->domain );
			...
			public function activate_strings() {
				...
				$this->strings['activation_failed']  = __( 'Plugin activation failed.', TGM_Nivoslider_Plugin_Activation::$instance->domain );
				$this->strings['activation_success'] = __( 'Plugin activated successfully.', TGM_Nivoslider_Plugin_Activation::$instance->domain );
	...
	if ( ! class_exists( 'TGM_Nivoslider_Bulk_Installer_Skin' ) ) {
		/**
		 ...
		 * @author Gary Jones <gamajo@gamajo.com>
		 */
		class TGM_Nivoslider_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
			...
			public function add_strings() {
				...
				if ( TGM_Nivoslider_Plugin_Activation::$instance->is_automatic ) {
					$this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', TGM_Nivoslider_Plugin_Activation::$instance->domain );
					$this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', TGM_Nivoslider_Plugin_Activation::$instance->domain ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', TGM_Nivoslider_Plugin_Activation::$instance->domain ) . '</span><span class="hidden">' . __( 'Hide Details', TGM_Nivoslider_Plugin_Activation::$instance->domain ) . '</span>.</a>';
					$this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', TGM_Nivoslider_Plugin_Activation::$instance->domain );
					$this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', TGM_Nivoslider_Plugin_Activation::$instance->domain );
				}
				/** Default installation strings */
				else {
					$this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', TGM_Nivoslider_Plugin_Activation::$instance->domain );
					$this->upgrader->strings['skin_update_failed_error']  = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', TGM_Nivoslider_Plugin_Activation::$instance->domain );
					$this->upgrader->strings['skin_update_failed']        = __( 'The installation of %1$s failed.', TGM_Nivoslider_Plugin_Activation::$instance->domain );
					$this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', TGM_Nivoslider_Plugin_Activation::$instance->domain ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', TGM_Nivoslider_Plugin_Activation::$instance->domain ) . '</span><span class="hidden">' . __( 'Hide Details', TGM_Nivoslider_Plugin_Activation::$instance->domain ) . '</span>.</a>';
					$this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', TGM_Nivoslider_Plugin_Activation::$instance->domain );
					$this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', TGM_Nivoslider_Plugin_Activation::$instance->domain );
				}
			...
			public function bulk_footer() {
				...
				foreach ( TGM_Nivoslider_Plugin_Activation::$instance->plugins as $plugin ) {
					if ( ! is_plugin_active( $plugin['file_path'] ) ) {
						echo '<p><a href="' . esc_url( add_query_arg( 'page', TGM_Nivoslider_Plugin_Activation::$instance->menu, admin_url( TGM_Nivoslider_Plugin_Activation::$instance->parent_url_slug ) ) ) . '" title="' . esc_attr( TGM_Nivoslider_Plugin_Activation::$instance->strings['return'] ) . '" target="_parent">' . __( TGM_Nivoslider_Plugin_Activation::$instance->strings['return'], TGM_Nivoslider_Plugin_Activation::$instance->domain ) . '</a></p>';
						$complete[] = $plugin;
						break;
					}
					/** Nothing to store */
				...
				if ( empty( $complete ) ) {
					echo '<p>' .  sprintf( TGM_Nivoslider_Plugin_Activation::$instance->strings['complete'], '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', TGM_Nivoslider_Plugin_Activation::$instance->domain ) . '">' . __( 'Return to the Dashboard', TGM_Nivoslider_Plugin_Activation::$instance->domain ) . '</a>' ) . '</p>';
					echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
				}
