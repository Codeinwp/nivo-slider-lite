jQuery( document ).ready(function($){
	...
	$( '.reattach-images' ).on('click', function(e){
		...
		if (post_id != '') {
			...
			$.ajax({
				...
				type: 'POST',
				dataType: 'json',
				data: { action:         nivo_plugin.labels.post_type + '_reattach_images',
					nonce:          nivo_plugin.nonce,
					post_id:    post_id
				},
				success: function(response){
	...
	$( '#activate-license' ).on('click', function(){
		...
		if (license_key != '') {
			...
			$.ajax({
				...
				type: 'POST',
				dataType: 'json',
				data: { action:         nivo_plugin.labels.post_type + '_activate_license',
					nonce:          nivo_plugin.nonce,
	...
	$( '#deactivate-license' ).on('click', function(){
		...
		$.ajax({
			...
			type: 'POST',
			dataType: 'json',
			data: { action: nivo_plugin.labels.post_type + '_deactivate_license',
				nonce:  nivo_plugin.nonce
			},
			success: function(response){
				if (response.error) {
